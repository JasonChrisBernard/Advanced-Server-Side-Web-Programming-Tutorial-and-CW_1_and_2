...
class Api_keys extends CI_Controller
{
    ...
    public function create()
    {
        ...
        $data = [
            ...
            'platformOptions' => $this->apiModel->getPlatformOptions(),
            'errors' => [],
            'old' => [
                'key_name' => '',
                'client_platform' => 'analytics_dashboard',
                'description' => ''
            ]
        ];
        ...
        if ($this->isPost()) {
            ...
            $clientPlatform = trim($this->input->post('client_platform', TRUE));
            ...
            if (!$this->apiModel->platformScopes($clientPlatform)) {
                $data['errors'][] = 'Invalid client platform selected.';
            }
            ...
            if (empty($data['errors'])) {
                ...
                if (!$result) {
                    $data['errors'][] = 'Could not generate API key.';
                } else {
                    $this->session->set_flashdata('plain_api_key', $result['plain_key']);
                    $this->session->set_flashdata('generated_scopes', $result['scopes']);
                    $this->session->set_flashdata('success', 'Scoped API key generated successfully. Copy it now because it will not be shown again.');
                    redirect('api-keys');
                    return;
                }
            }
