...
- CW2 staff analytics:
  - routes: `src/routes/staffRoutes.js`
  - controller: `src/controllers/staffController.js`
  - model: `src/models/staffAnalyticsModel.js`
...
3. The middleware checks the endpoint's required scope, such as `read:alumni`, `read:analytics`, or `read:alumni_of_day`.
4. On success, usage is logged and the request proceeds.
5. `developerApiController.js` calls either `staffAnalyticsModel.js` for dashboard data or `featuredAlumnusService.js` for alumnus-of-day data.

### CW2 staff dashboard flow

1. Run the server with `npm run start:cw2` so it listens on `localhost:3001`.
2. A seeded staff/developer account logs in and is redirected to `/staff/dashboard`.
3. `staffController.js` reads KPI cards, graph data, alumni filters, and API usage logs from `staffAnalyticsModel.js`.
4. `/staff/alumni` filters alumni by programme, graduation date/year, and industry sector.
5. `/staff/graphs` renders chart canvases for programme, graduation, industry, bidding, and API endpoint usage.
...
- API keys are scoped per client application, so Analytics Dashboard keys cannot access Mobile AR App endpoints and Mobile AR App keys cannot access analytics endpoints
