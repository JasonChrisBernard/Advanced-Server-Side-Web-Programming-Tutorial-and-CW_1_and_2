...
// CW2 runs as a separate staff-only dashboard on port 3001.
const STAFF_DASHBOARD_ROLES = new Set(["staff"]);

// Detect the CW2 staff dashboard instance from environment/port.
function isStaffDashboardMode() {
  return (
    process.env.APP_MODE === "cw2_staff" ||
    Number(process.env.PORT || 3000) === 3001
  );
}

// Keep the staff dashboard limited to staff accounts.
function isAllowedOnCurrentPortal(user) {
  if (!isStaffDashboardMode()) {
    return true;
  }

  return STAFF_DASHBOARD_ROLES.has(user.role);
}
...
exports.register = async (req, res) => {
  try {
    ...
    const registeringStaff = isStaffDashboardMode();
    // Staff registration is local to the staff dashboard and can log in immediately.
    const verificationToken = registeringStaff
      ? null
      : crypto.randomBytes(32).toString("hex");
    // Set an expiry time so verification links are not valid forever.
    const verificationTokenExpiresAt = registeringStaff
      ? null
      : new Date(Date.now() + VERIFICATION_TOKEN_TTL_MINUTES * 60 * 1000).toISOString();
    ...
    User.createUser({
      ...
      role: registeringStaff ? "staff" : "alumnus",
      isVerified: registeringStaff ? 1 : 0,
    });

    if (registeringStaff) {
      return res.render("auth/message", {
        title: "Staff registration complete",
        message: "Staff account created. You can now sign in to the staff dashboard.",
      });
    }
...
exports.login = async (req, res) => {
  try {
    ...
    if (!isAllowedOnCurrentPortal(user)) {
      return res.render("auth/login", {
        error: "This portal accepts staff accounts only.",
      });
    }
    ...
    return req.session.regenerate((sessionError) => {
      ...
      // Send staff directly to the CW2 dashboard and alumni to the CW1 home page.
      return res.redirect(isStaffDashboardMode() ? "/staff/dashboard" : "/");
    });
  } catch (error) {
...
exports.logout = (req, res) => {
  ...
  req.session.destroy(() => {
    const defaultCookieName = Number(process.env.PORT || 3000) === 3001
      ? "cw2.staff.sid"
      : "alumni.sid";
    res.clearCookie(process.env.SESSION_NAME || defaultCookieName);
    res.redirect("/login");
  });
};
