...
// Show the logged-in API key management page.
router.get("/developer-portal", requireRole(["developer", "staff"]), developerApiController.showDeveloperPortal);
// Create a new external client and bearer token from the portal form.
router.post(
  "/developer-portal/clients",
  requireRole(["developer", "staff"]),
...
// Protect the staff alumni directory endpoint with an analytics-dashboard scoped bearer token.
router.get(
  "/api/v1/alumni",
  requireBearerToken("read:alumni"),
  developerApiController.getAlumniDirectoryApi
);

// Protect aggregate analytics with the read:analytics scope.
router.get(
  "/api/v1/analytics/summary",
  requireBearerToken("read:analytics"),
  developerApiController.getAnalyticsSummaryApi
);

// Protect the mobile/AR alumnus-of-day endpoint with its own isolated scope.
router.get(
  "/api/v1/alumni-of-day",
  requireBearerToken("read:alumni_of_day"),
  developerApiController.getAlumnusOfDayApi
);

// Keep the original featured alumnus endpoint as a compatibility alias for alumnus-of-day clients.
router.get(
