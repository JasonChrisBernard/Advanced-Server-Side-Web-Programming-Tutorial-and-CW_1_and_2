...
// Import bcrypt so a verified local staff dashboard account can be seeded for CW2 demos.
...
// Rebuild older users tables whose role CHECK constraint does not include the dedicated staff role.
function migrateUsersRoleConstraintIfNeeded() {
  const usersTable = db
    .prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'users'")
    .get();

  if (!usersTable || String(usersTable.sql || "").includes("'staff'")) {
    return;
  }
  ...
  db.exec(`
    ...
    CREATE TABLE users_role_migration (
      ...
      role TEXT NOT NULL DEFAULT 'alumnus' CHECK(role IN ('developer', 'alumnus', 'staff')),
...
// Seed verified local staff-dashboard users so CW2 can be demonstrated separately on port 3001.
function synchronizeConfiguredStaffPortalUsers() {
  const configuredStaffEmails = String(process.env.STAFF_PORTAL_EMAILS || "")
    .split(",")
    .map((email) => email.trim().toLowerCase())
    .filter(Boolean);
  const defaultPassword = String(process.env.STAFF_PORTAL_DEFAULT_PASSWORD || "");

  if (configuredStaffEmails.length === 0 || !defaultPassword) {
    return;
  }
  ...
  const findExistingStaffUser = db.prepare("SELECT id FROM users WHERE lower(email) = ?");
  const insertStaffUser = db.prepare(`
    ...
    VALUES (?, ?, ?, 'staff', 1, NULL, NULL, NULL, ?, ?)
  `);
  const promoteExistingStaffUser = db.prepare(`
    UPDATE users
    SET role = 'staff',
        is_verified = 1,
        verification_token = NULL,
  ...
  configuredStaffEmails.forEach((email, index) => {
    const existingUser = findExistingStaffUser.get(email);

    if (existingUser) {
      promoteExistingStaffUser.run(now, email);
      return;
    }

    insertStaffUser.run(
      index === 0 ? "CW2 Staff Dashboard" : `CW2 Staff Dashboard ${index + 1}`,
      email,
      passwordHash,
      now,
      now
...
// Populate new CW2 dashboard fields from existing CW1 degree/employment data when possible.
function backfillStaffAnalyticsProfileFields() {
  if (
    !tableExists("alumni_profiles") ||
    !tableExists("alumni_profile_degrees") ||
    !tableExists("alumni_profile_employment")
  ) {
    return;
  }
  ...
  db.prepare(`
    ...
    WHERE (programme IS NULL OR TRIM(programme) = '')
      AND EXISTS (
        SELECT 1
        FROM alumni_profile_degrees d
        WHERE d.profile_id = alumni_profiles.id
  ...
  db.prepare(`
    UPDATE alumni_profiles
    SET graduation_date = (
      SELECT d.completion_date
      FROM alumni_profile_degrees d
      WHERE d.profile_id = alumni_profiles.id
        AND d.completion_date IS NOT NULL
        AND TRIM(d.completion_date) <> ''
      ORDER BY d.display_order ASC, d.id ASC
      LIMIT 1
    )
    WHERE (graduation_date IS NULL OR TRIM(graduation_date) = '')
      AND EXISTS (
        SELECT 1
        FROM alumni_profile_degrees d
        WHERE d.profile_id = alumni_profiles.id
  ...
  const profilesMissingIndustry = db.prepare(`
    SELECT
      p.id,
    ...
    LEFT JOIN alumni_profile_employment e
      ON e.profile_id = p.id
      AND e.id = (
        SELECT latest.id
        FROM alumni_profile_employment latest
        WHERE latest.profile_id = p.id
        ORDER BY latest.display_order ASC, latest.id ASC
        LIMIT 1
      )
    WHERE p.industry_sector IS NULL OR TRIM(p.industry_sector) = ''
  `).all();
  const updateIndustry = db.prepare(`
    UPDATE alumni_profiles
    SET industry_sector = ?,
        updated_at = ?
    WHERE id = ?
  ...
  profilesMissingIndustry.forEach((profile) => {
    const text = [
    ...
    if (/data|analytics|machine learning|ai|artificial intelligence/.test(text)) {
      industrySector = "Data & AI";
    } else if (/security|cyber/.test(text)) {
      industrySector = "Cybersecurity";
    } else if (/cloud|devops|network|infrastructure|platform/.test(text)) {
      industrySector = "Cloud & Infrastructure";
    } else if (/design|ux|ui/.test(text)) {
      industrySector = "Design & UX";
    } else if (/manager|management|business|product|consult/.test(text)) {
      industrySector = "Business & Management";
    } else if (/software|engineer|developer|web|mobile|full stack|backend|frontend/.test(text)) {
      industrySector = "Software Engineering";
    }

    updateIndustry.run(industrySector, now, profile.id);
  });
}
...
ensureColumnExists("users", "role", "TEXT NOT NULL DEFAULT 'alumnus' CHECK(role IN ('developer', 'alumnus', 'staff'))");
migrateUsersRoleConstraintIfNeeded();
migrateLegacyAuthTokensToHashes();
synchronizeDeveloperRoles();
synchronizeConfiguredStaffPortalUsers();
...
// Ensure CW2 staff-dashboard filter fields exist when an older CW1 database is reused.
ensureColumnExists("alumni_profiles", "programme", "TEXT");
ensureColumnExists("alumni_profiles", "graduation_date", "TEXT");
ensureColumnExists("alumni_profiles", "industry_sector", "TEXT");
...
// Reuse existing CW1 profile sections to fill the new CW2 analytics fields on older databases.
backfillStaffAnalyticsProfileFields();
...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_alumni_profiles_programme
    ON alumni_profiles (programme);
  CREATE INDEX IF NOT EXISTS idx_alumni_profiles_graduation_date
    ON alumni_profiles (graduation_date);
  CREATE INDEX IF NOT EXISTS idx_alumni_profiles_industry_sector
    ON alumni_profiles (industry_sector);
